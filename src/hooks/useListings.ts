...
function transformMockListing(listing: typeof mockListings[0]): ScraperListing {
  return {
    ...
    licensePlate: null,
    licensePlateHash: null,
    optionsRaw: null,
    dealerId: listing.dealerId,
    dealerName: listing.dealer,
    dealerCity: null,
    status: listing.status === 'active' ? 'active' : 'gone_suspected',
    firstSeenAt: listing.firstSeen,
    lastSeenAt: listing.lastUpdated,
    goneDetectedAt: null,
    soldConfirmedAt: null,
...
function transformDBListing(row: {
  ...
  return {
    ...
    licensePlateHash: (row as any).license_plate_hash || null,
    optionsRaw: row.options_raw,
    dealerId: row.dealer_id,
    dealerName: row.dealer_name,
    dealerCity: row.dealer_city,
    status: row.status as 'active' | 'gone_suspected' | 'sold_confirmed' | 'returned',
    firstSeenAt: row.first_seen_at,
    lastSeenAt: row.last_seen_at,
    goneDetectedAt: (row as any).gone_detected_at || null,
    soldConfirmedAt: (row as any).sold_confirmed_at || null,
    daysOnMarket,
    ...
    priceBucket: (row as any).price_bucket || null,
    mileageBucket: (row as any).mileage_bucket || null,
    outboundSources: (row as any).outbound_sources || [],
  };
}
